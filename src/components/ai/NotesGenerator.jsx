...
export default function NotesGenerator() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (ev) => setTopic(ev.target.result.slice(0, 500))
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await callGemini(`Generate comprehensive study notes on the topic: "${topic}". Format with markdown headers (##), bullet points, definitions with bold terms, real-world examples, and a summary section at the end. Make it detailed and well-structured for exam preparation.`)
      if (res.error) {
        setError(res.error)
        setLoading(false)
        return
      ...
      setError('Failed to generate notes. Check your API key and try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-4">
      <ToolGuide
        steps={['Enter a topic (or upload a .txt file)', 'Click "Generate"', 'Review your AI-generated study notes', 'Copy or download for later use']}
        tips={['Supports .txt file upload for longer content', 'Notes include key concepts, summaries, and study points']}
      />
      <motion.div variants={item} className="flex gap-2">
        <div className="relative flex-1">
          <Input
            ...
            placeholder="Enter a topic or upload a .txt file..."
            className="w-full pr-10"
          />
          <button
            onClick={() => fileRef.current?.click()}
            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg bg-[var(--accent)]/20 text-[var(--accent)] hover:bg-[var(--accent)]/30 transition-all duration-200"
          >
            <Upload size={14} />
          </button>
          <input ref={fileRef} type="file" accept=".txt" onChange={handleFileUpload} className="hidden" />
        </div>
        <Button onClick={handleGenerate} loading={loading} icon={FileText} className="group relative overflow-hidden">
        ...
        {loading && (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <GlassCard className="p-8 text-center">
              ...
              <p className="text-sm text-muted mt-3">Generating study notes...</p>
            </GlassCard>
          </motion.div>
        )}
          ...
          <motion.div key="result" variants={item} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }}>
            <GlassCard className="p-5 relative overflow-hidden">
              ...
              <motion.div
                ...
                className="relative z-10 text-sm text-primary whitespace-pre-wrap leading-relaxed prose prose-sm max-w-none"
              >
                {output.split('\n').map((line, i) => (
                  <motion.span
                    key={i}
